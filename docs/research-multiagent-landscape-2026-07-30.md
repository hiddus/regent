...
>
> **口径更新（2026-08-11，定义 3.0 取代）**：成稿时「多 Agent 非默认 / 须证明净收益 / ROLLOUT_NOT_ALLOWED」等表述，按现行 [`REGENT-DEFINITION-3.0`](definitions/REGENT-DEFINITION-3.0.txt) **只约束生产默认晋级与不可逆现实权限扩张**；沙箱候选拓扑、角色创造与组织试验默认开放（ATTRIBUTE_2/4/6/7）。下文外部证据仍可用，门禁适用范围已收窄。
...
> **口径更新（2026-08-11 补强）**：净收益 / 冻结实验 / `ROLLOUT_NOT_ALLOWED` **只约束生产晋级与不可逆现实权限扩张**；沙箱探索、候选拓扑与组织试验不在此门禁内。下表「外部证据」仍成立。

| Regent 冻结原则（当时口径） | 外部印证 |
|---|---|
...
**结论：Regent 的治理姿态领先于行业平均实践约一年；不要因框架热度动摇「单 Agent 作为生产默认对照基线」叙事——这约束的是生产晋级与现实权限，不是沙箱探索禁令（定义 3.0）。**
...
5. **用任务属性先验初始化 `UtilityFunction U(O_t)` / `TopologyPlanner`**：以可测特征（工具数量、子任务可分解性、顺序依赖强度、单 Agent 基线成功率）作路由输入；显式编码三条经验律——
   - 单 Agent 基线 >45% → 仅生产默认推荐降权，不禁止沙箱候选；
   - 强顺序依赖 → 生产默认偏向单 Agent，探索空间仍可保留多 Agent；
   - 可分解 + 需验证 → 生产默认偏向「中心化 + 验证瓶颈」拓扑，不删除其他沙箱候选。
   这把 87% 预测器思想变成生产默认推荐偏置（非 OrganizationSpace 探索禁令），降低默认误选成本同时保留沙箱试验。
