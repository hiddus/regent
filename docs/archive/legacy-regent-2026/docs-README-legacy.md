...
| [`definitions/REGENT-DEFINITION-3.0.txt`](definitions/REGENT-DEFINITION-3.0.txt) | FROZEN | **唯一规范定义源** + `.sha256`；CI 防漂移。1.0 / 2.0 已被取代，只读保留，见 [`definitions/README.md`](definitions/README.md) |
...
| [`m6-canary-watch-plan-2026-08-01.md`](m6-canary-watch-plan-2026-08-01.md) | **HALTED_PENDING_QUALIFICATION** | M6 观察窗已 halt；须先 Offline Qual；扩流禁止 |
| [`m6-canary-window-2026-08-01.json`](m6-canary-window-2026-08-01.json) | RECORD | **CLAMPED_PENDING_QUALIFICATION**（曾开 5% 后已 clamp；gate=false / percent=0） |
...
| [`decision-record-definition-supersession-2026-08-11.md`](decision-record-definition-supersession-2026-08-11.md) | NOTE | 历史 DecisionRecord 的 definition_id 不改写；现行规范源为 3.0 |
| [`pptx-source-update-note-2026-08-11.md`](pptx-source-update-note-2026-08-11.md) | NOTE | jsx/html 幻灯片源已更新；二进制 pptx 待重导 |
| [`decision-note-gq4-pending-2026-07-31.md`](decision-note-gq4-pending-2026-07-31.md) | PENDING | GQ-4 未晋级；代码默认 agentic ≠ GQ-4 正式晋级 |
...
永久定义只存在于 `docs/definitions/REGENT-DEFINITION-3.0.txt`，后续文档只能引用，不得改写。1.0 / 2.0 是已被取代的历史版本：文档若仍以它们为规范源，读者会拿到被 3.0 撤回的规则（例如 1.0 ATTRIBUTE_7 的"明确终态"已被 3.0 的"边界落在现实影响上"取代）。
...
2. **当前 M6 状态**：**CLAMPED_PENDING_QUALIFICATION / HALTED**（canary off；next Offline Qual；[`m6-canary-window-2026-08-01.json`](m6-canary-window-2026-08-01.json)、[`m6-canary-watch-plan-2026-08-01.md`](m6-canary-watch-plan-2026-08-01.md)）；参考 [`agent-core-restoration-executable-plan-2026-08-01.md`](agent-core-restoration-executable-plan-2026-08-01.md)
3. Technical-Spec §13.7.3 / §13.8.3 / §18.6 + 三份附录（实现合同时查阅）
4. CD-6…12 / `Regent-Plan.md` §14 仍为工程参照，但生产 canary 状态以 M6 窗记录为准（当前为 clamped，非 ACTIVE 5%）
5. **禁止**在 Graduation + 文档 CURRENT 前读 P2-1 并开工 Scheduler
6. **禁止**将历史 M6 开窗或当前 clamp 状态宣称成 GQ-4；重新开窗 / 扩 10% / 默认 agentic 须过 Offline Qual + 观察护栏 + DecisionRecord
