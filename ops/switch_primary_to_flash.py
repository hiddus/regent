"""Switch production primary model to tencent/deepseek-v4-flash.

Key / base URL unchanged. Updates env files and recreates api + workers
(docker cannot change env in place).

Usage:
  python ops/switch_primary_to_flash.py            # dry-run
  python ops/switch_primary_to_flash.py --execute
"""
...
PRIMARY = "tencent/deepseek-v4-flash"
...
print(json.dumps({"recreated": targets, "after_live": live_env("regent-worker")}, ensure_ascii=False))
'''
...
def main() -> int:
    ...
    try:
        ...
        with sftp.file("/tmp/_switch_to_flash.py", "w") as fh:
            fh.write(remote)
        sftp.close()
        print(f"[{'EXECUTE' if args.execute else 'DRY-RUN'}] primary -> {PRIMARY}")
        stdin, stdout, stderr = client.exec_command(
            "python3 /tmp/_switch_to_flash.py", timeout=600
        )
